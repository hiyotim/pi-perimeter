...
- No host deletion or rename API. Deletions/renames inside staging must not silently erase or rename host objects. Clearly report unsupported/ignored effects and any separately authorized creates/replacements. Define nonzero-exit, cancellation, timeout, crash and partial-export behavior; no false all-or-nothing claim. Establish process-tree quiescence before export and keep captured output bytes sealed through authorization and effect; a surviving child must not mutate the approved export source. If quiescence cannot be established, refuse export. Cleanup touches only invocation-owned artifacts and leaves no writable authority accessible to descendants.
