...
- Only the dedicated release workflow (`.github/workflows/release.yml`) may publish, and only on a tag push: it rebuilds the staging artifact deterministically, verifies its bytes against the version-bound release manifest (`docs/release-hashes.json` for `1.0.0`, `docs/release-hashes-1.0.1-final.json` for `1.0.1`, `docs/release-hashes-<version>.json` otherwise), pins an explicit npm 11.x release (>= 11.5.1) for OIDC support, and runs `npm publish --provenance` from inside `release-staging` over npm Trusted Publishing (GitHub OIDC, `id-token: write`) with no publish token and no secret reference. A manual `workflow_dispatch` run of the same workflow is verify-only — the publish step is gated to tag push — so it rebuilds, verifies, and dry-run packs the candidate without publishing.
