...
**Release gate:** installation, rollback, compatibility, disclosure, and known limitations are documented and tested; publication requires explicit maintainer action. (Four checklist items are closed; the release-candidate security/documentation reviews remain open, so this gate is not met and Phase 6 does not advance overall. Publication additionally requires an explicit maintainer decision that has not been taken.)
