...
**Release gate:** v1.0 is released only after the implementation and regression suite support a stable, narrowly worded set of security guarantees. (Closed 2026-09-24 by explicit owner decision as macOS-only v1.0: declared target only; Linux/Windows explicitly unsupported with R4 open; evidence is the four step acceptances in [STATE.md](STATE.md). Publication remains a separate explicit maintainer decision.)
