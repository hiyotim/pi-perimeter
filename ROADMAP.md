...
**Release gate:** installation, rollback, compatibility, disclosure, and known limitations are documented and tested; publication requires explicit maintainer action. (All five checklist items are closed; the gate itself remains open pending an explicit maintainer release decision — checklist closure alone does not meet it. Publication additionally requires an explicit maintainer decision that has not been taken.)
