...
interface EditDecisionFixture {
  readonly root: string;
  readonly workspace: string;
  readonly outside: string;
  resolve(requestedPath: string): Promise<ResolvedPath>;
  evaluate(requestedPath: string): Promise<EditPathDecision>;
}
...
async function withFixture(
  run: (fixture: EditDecisionFixture) => Promise<void>,
): Promise<void> {
  const root = await mkdtemp(path.join(os.tmpdir(), "pi-warden-edit-decisions-"));
  ...
  try {
    ...
    await run({
      ...
      evaluate: async (requestedPath) =>
        evaluateEditPath("edit", await resolve(requestedPath)),
    });
  } finally {
...
test("emits the complete selected result for every table row and overlap", async () => {
  await withFixture(async ({ outside, evaluate }) => {
    const cases: readonly {
      ...
      readonly expected: EditPathDecision;
    }[] = [
      {
        path: "ordinary.txt",
        expected: { decision: "ALLOW", reason: "WORKSPACE_EDIT" },
      },
      {
        path: "src",
        expected: { decision: "ALLOW", reason: "WORKSPACE_EDIT" },
      ...
      },
      {
        ...
        expected: { decision: "ASK", reason: "EXTERNAL_EDIT" },
      },
      {
        path: "escape.txt",
        expected: { decision: "ASK", reason: "EXTERNAL_EDIT" },
      ...
      {
        path: "missing.txt",
        expected: { decision: "DENY", reason: "EDIT_TARGET_MISSING" },
      },
      {
...
test("denies every ordinary missing target and keeps canonical membership for existing targets", async () => {
  await withFixture(async ({ outside, resolve }) => {
    ...
    assert.deepEqual(evaluateEditPath("edit", insideExisting), {
      decision: "ALLOW",
      reason: "WORKSPACE_EDIT",
    });
    assert.deepEqual(evaluateEditPath("edit", insideMissing), {
      decision: "DENY",
      reason: "EDIT_TARGET_MISSING",
    });
    assert.deepEqual(evaluateEditPath("edit", insideNestedMissing), {
      decision: "DENY",
      reason: "EDIT_TARGET_MISSING",
    });
    assert.deepEqual(evaluateEditPath("edit", externalExisting), {
      decision: "ASK",
      reason: "EXTERNAL_EDIT",
    });
    assert.deepEqual(evaluateEditPath("edit", externalMissing), {
      decision: "DENY",
      reason: "EDIT_TARGET_MISSING",
    });
  });
  ...
  await withFixture(async ({ outside, resolve }) => {
    ...
    for (const [index, operation] of unsupported.entries()) {
      for (const [label, resource] of [
        ...
        assert.deepEqual(
          evaluateEditPath(operation as unknown as "edit", resource),
    ...
    assert.deepEqual(
      evaluateEditPath(coercingOperation as unknown as "edit", ordinary),
      { decision: "DENY", reason: "UNSUPPORTED_OPERATION" },
    );
  });
});
...
test("requires genuine resolver issuance before any resource property access", async () => {
  await withFixture(async ({ workspace, resolve }) => {
    ...
    assert.deepEqual(evaluateEditPath("edit", genuine), {
      decision: "ALLOW",
      reason: "WORKSPACE_EDIT",
    });
    ...
    const forged: readonly (readonly [string, unknown])[] = [
      ...
      [
        ...
        { ...claimed, insideWorkspace: true, targetExists: false },
      ],
      [
        "claimed external and existing flags",
        { ...claimed, insideWorkspace: false, targetExists: true },
      ],
      [
        "claimed external and missing flags",
    ...
    for (const [label, candidate] of forged) {
      assert.deepEqual(
        evaluateEditPath("edit", candidate as ResolvedPath),
...
test("returns exactly the decision and reason fields synchronously", async () => {
  await withFixture(async ({ outside, resolve }) => {
    const results: readonly EditPathDecision[] = [
      evaluateEditPath("edit", await resolve("ordinary.txt")),
      evaluateEditPath("edit", await resolve(path.join(outside, "ordinary.txt"))),
      evaluateEditPath("edit", await resolve("missing.txt")),
      evaluateEditPath("edit", await resolve(".env")),
      evaluateEditPath("edit", await resolve("private.key")),
      evaluateEditPath("read" as unknown as "edit", await resolve("ordinary.txt")),
      evaluateEditPath("edit", {} as unknown as ResolvedPath),
    ];
  ...
  await withFixture(async ({ workspace, outside }) => {
    ...
    await assert.rejects(
      resolveWorkspacePath(workspace, "broken-link").then((resolved) => {
        ...
        return evaluateEditPath("edit", resolved);
      }),
      (error: unknown) =>
...
test("rejects non-edit operations, structural resources, and invalid combinations at compile time", () => {
  if (false) {
    const resource = null as unknown as ResolvedPath;
    const decision = null as unknown as EditPathDecision;
    const structural = {
      requestedPath: "ordinary.txt",
    ...
    const allow: EditPathDecision = {
      decision: "ALLOW",
      reason: "WORKSPACE_EDIT",
    };
    const ask: EditPathDecision = {
      decision: "ASK",
      reason: "EXTERNAL_EDIT",
    };
    const denyInvalid: EditPathDecision = {
      decision: "DENY",
      reason: "INVALID_RESOURCE",
    };
    const denyUnsupported: EditPathDecision = {
      decision: "DENY",
      reason: "UNSUPPORTED_OPERATION",
    };
    const denySecret: EditPathDecision = {
      decision: "DENY",
      reason: "SECRET_RESOURCE",
    };
    const denySensitive: EditPathDecision = {
      decision: "DENY",
      reason: "SENSITIVE_RESOURCE",
    };
    const denyMissing: EditPathDecision = {
      decision: "DENY",
      reason: "EDIT_TARGET_MISSING",
    };
    const denyReasons: readonly EditPathDenyReason[] = [
      "INVALID_RESOURCE",
    ...
    // @ts-expect-error only the exact edit operation is admitted
    evaluateEditPath("read", resource);
    // @ts-expect-error write is not the edit operation
    evaluateEditPath("write", resource);
    // @ts-expect-error delete is not the edit operation
    evaluateEditPath("delete", resource);
    // @ts-expect-error rename is not the edit operation
    evaluateEditPath("rename", resource);
    // @ts-expect-error shell operations are not the edit operation
    evaluateEditPath("bash", resource);
    // @ts-expect-error case variants are not the exact edit operation
    evaluateEditPath("Edit", resource);
    // @ts-expect-error absent runtime values are not admitted
    evaluateEditPath(undefined, resource);
    // @ts-expect-error structural resources cannot satisfy resolver issuance
    evaluateEditPath("edit", structural);

    // @ts-expect-error result fields are readonly
    decision.decision = "ALLOW";
    // @ts-expect-error result fields are readonly
    decision.reason = "WORKSPACE_EDIT";

    // @ts-expect-error ALLOW only pairs with WORKSPACE_EDIT
    const invalidAllow: EditPathDecision = { decision: "ALLOW", reason: "EXTERNAL_EDIT" };
    // @ts-expect-error ASK only pairs with EXTERNAL_EDIT
    const invalidAsk: EditPathDecision = { decision: "ASK", reason: "WORKSPACE_EDIT" };
    // @ts-expect-error DENY cannot carry WORKSPACE_EDIT
    const invalidDeny: EditPathDecision = { decision: "DENY", reason: "WORKSPACE_EDIT" };
    // @ts-expect-error read-only reasons are not part of the edit contract
    const readReason: EditPathDecision = { decision: "DENY", reason: "READ_TARGET_MISSING" };
    // @ts-expect-error write-only reasons are not part of the edit contract
    const writeReason: EditPathDecision = { decision: "DENY", reason: "WORKSPACE_WRITE" };
    // @ts-expect-error no SANDBOX outcome is part of the contract
    const sandbox: EditPathDecision = { decision: "SANDBOX", reason: "WORKSPACE_EDIT" };
    // @ts-expect-error the result has exactly decision and reason
    const extra: EditPathDecision = { decision: "ALLOW", reason: "WORKSPACE_EDIT", extra: true };
